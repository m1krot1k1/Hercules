# Hercules Agent Security Policy

This document describes Hercules Agent's trust model, names the one
...
Hercules Agent is a single-tenant personal agent. Its posture is
layered, and the layers are not equally load-bearing. Reporters and
operators should reason about them in the same terms.

### 2.1 Definitions

- **Agent process.** The Python interpreter running Hercules Agent,
...
- **Trust envelope.** The set of resources an operator has implicitly
  granted Hercules Agent access to by running it — typically, whatever
  the operator's own user account can reach on the host.
- **Stance.** An explicit statement in Hercules Agent's documentation
...
Hercules Agent supports two OS-level isolation postures. They address
...
Hercules Agent supports this in two ways:

- **Hercules Agent's own Docker image and Compose setup.** Lighter-
...
Under a whole-process wrapper, Hercules Agent's in-process heuristics
...
Hercules Agent filters the environment it passes to its lower-trust
...
A malicious or buggy plugin is not a vulnerability in Hercules Agent
itself. Bugs in Hercules Agent's plugin-install or plugin-discovery
...
4. **Within the authorized set, all callers are equally trusted.**
   Hercules Agent does not model per-caller capabilities inside a
...
- Trust-model documentation violations: code behaving contrary to
  what this policy, Hercules Agent's own documentation, or reasonable
  operator expectations would predict — including cases where
  Hercules Agent has documented a stance about how its output should
...
- **Community-contributed skills and plugins.** Third-party skills
  ...
  plugins are in the operator's review surface, not Hercules Agent's
  trust surface (§2.4, §2.5). A skill or plugin doing something
  malicious is the expected failure mode of one that wasn't
  reviewed, not a vulnerability in Hercules Agent. Bugs in Hermes
...
- Hercules Agent includes supply-chain guards for MCP server
