...
Usage:
    ...
    hermes uninstall           Uninstall Hercules Agent
...
def cmd_whatsapp(args):
    ...
    if (session_dir / "creds.json").exists():
        ...
        if wa_mode == "bot":
            ...
            print()
            print("  Tip: Agent responses are prefixed with '⚕ Hercules Agent'")
        else:
            print("  Next steps:")
            print("    1. Start the gateway:  hermes gateway")
...
def cmd_postinstall(args):
    ...
    print("⚕ Hercules post-install bootstrap")
    print()
...
def cmd_version(args):
    """Show version."""
    print(f"Hercules Agent v{__version__} ({__release_date__})")
...
def _update_via_zip(args):
    """Update Hercules Agent by downloading a ZIP archive.
    ...
    branch = "dev"
    zip_url = (
        f"https://github.com/m1krot1k1/Hercules/archive/refs/heads/{branch}.zip"
    )
...
OFFICIAL_REPO_URLS = {
    "https://github.com/m1krot1k1/Hercules.git",
    "git@github.com:m1krot1k1/Hercules.git",
    "https://github.com/m1krot1k1/Hercules",
    "git@github.com:m1krot1k1/Hercules",
}
OFFICIAL_REPO_URL = "https://github.com/m1krot1k1/Hercules.git"
...
def _sync_with_upstream_if_needed(git_cmd: list[str], cwd: Path) -> None:
    ...
    - Compare origin/dev with upstream/dev
    - If origin/dev is strictly behind upstream/dev, pull from upstream
    ...
    if not has_upstream:
        ...
        print("  This means you may miss updates from m1krot1k1/Hercules.")
        print()
        try:
            response = (
        ...
        if response in {"", "y", "yes"}:
            ...
            if _add_upstream_remote(git_cmd, cwd):
                print(
                    "  ✓ Added upstream: https://github.com/m1krot1k1/Hercules.git"
                )
                has_upstream = True
            else:
            ...
            print(
                "  Skipped. Run 'git remote add upstream https://github.com/m1krot1k1/Hercules.git' to add later."
            )
    ...
    # Compare origin/dev with upstream/dev
    origin_ahead = _count_commits_between(git_cmd, cwd, "upstream/dev", "origin/dev")
    upstream_ahead = _count_commits_between(
        git_cmd, cwd, "origin/dev", "upstream/dev"
    )
    ...
    # If origin/dev has commits not on upstream, don't trample
    if origin_ahead > 0:
        print()
        print(f"ℹ Your fork has {origin_ahead} commit(s) not on upstream.")
        print("  Skipping upstream sync to preserve your changes.")
        print("  If you want to merge upstream changes, run:")
        print("    git pull upstream dev")
        return
    ...
    # origin/dev is strictly behind upstream/dev (can fast-forward)
    print()
    ...
    try:
        subprocess.run(
            git_cmd + ["pull", "--ff-only", "upstream", "dev"],
            cwd=cwd,
            check=True,
        )
    except subprocess.CalledProcessError:
...
def cmd_update(args):
    """Update Hercules Agent to the latest version.
    ...
    if is_managed():
        managed_error("update Hercules Agent")
        return
...
def _cmd_update_impl(args, gateway_mode: bool):
    ...
    try:
        ...
        # Always update against dev
        branch = "dev"

        # If user is on a non-dev branch or detached HEAD, switch to dev
        if current_branch != "dev":
            ...
            print(f"  ⚠ Currently on {label} — switching to dev for update...")
        ...
        if commit_count == 0:
            ...
            if current_branch not in {"dev", "HEAD"}:
                subprocess.run(
        ...
        try:
            ...
            if pull_result.returncode != 0:
                ...
                if reset_result.returncode != 0:
                    ...
                    print(
                        "  Try manually: git fetch origin && git reset --hard origin/dev"
                    )
                    sys.exit(1)
        ...
        # Fork upstream sync logic (only for dev branch on forks)
        if is_fork and branch == "dev":
...
def main():
    ...
    setup_parser = subparsers.add_parser(
        ...
        description="Configure Hercules Agent with an interactive wizard. "
    ...
    status_parser = subparsers.add_parser(
        ...
        description="Display status of Hercules Agent components",
    )
    ...
    debug_parser = subparsers.add_parser(
        ...
        description="Debug utilities for Hercules Agent. Use 'hermes debug share' to "
...
""",
    ...
    config_parser = subparsers.add_parser(
        ...
        description="Manage Hercules Agent configuration",
    )
    ...
    uninstall_parser = subparsers.add_parser(
        "uninstall",
        help="Uninstall Hercules Agent",
        description="Remove Hercules Agent from your system. Can keep configs/data for reinstall.",
    )
    ...
    acp_parser = subparsers.add_parser(
        "acp",
        help="Run Hercules Agent as an ACP (Agent Client Protocol) server",
        description="Start Hercules Agent in ACP mode for editor integration (VS Code, Zed, JetBrains)",
    )
    ...
    def cmd_acp(args):
        """Launch Hercules Agent as an ACP server."""
        try:
    ...
    dashboard_parser = subparsers.add_parser(
        ...
        description="Launch the Hercules Agent web dashboard for managing config, API keys, and sessions",
    )
