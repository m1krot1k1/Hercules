...
hermes cron create "0 2 * * *" \
  "You are a project manager triaging the m1krot1k1/Hercules GitHub repo.

1. Run: gh issue list --repo m1krot1k1/Hercules --state open --json number,title,labels,author,createdAt --limit 30
...
hermes cron create "0 9 * * 1" \
  "Scan the m1krot1k1/Hercules repo for documentation drift.

1. Run: gh pr list --repo m1krot1k1/Hercules --state merged --json number,title,files,mergedAt --limit 30
...
```bash
hermes cron create "0 8 * * *" \
  "Search arXiv for the 3 most interesting papers on 'language model reasoning' OR 'tool-use agents' from the past day. For each paper, create an Obsidian note with the title, authors, abstract summary, key contribution, and potential relevance to Hercules Agent development." \
