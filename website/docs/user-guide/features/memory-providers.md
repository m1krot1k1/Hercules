...
Hercules Agent ships with 8 external memory provider plugins that give the agent persistent, cross-session knowledge beyond the built-in MEMORY.md and USER.md. Only **one** external provider can be active at a time — the built-in memory is always active alongside it.
...
| `retain_context` | `conversation between Hercules Agent and the User` | Context label for retained memories |
...
See [plugin README](https://github.com/m1krot1k1/Hercules/blob/main/plugins/memory/hindsight/README.md) for the full configuration reference.
